            ...
            <table class="coupon-table">
              ...
              <tbody>
                ...
                >
                  ...
                  <td>{{ issuedCoupon.coupon_use_status }}</td>
                  <td>{{ issuedCoupon.discount_rate }}</td>
...
import IssuedCouponFilter from '@/components/marketing/IssuedCouponFilter.vue';
  ...
  const applyFilters = async (filters) => {
    try {
      const response = await axios.post('http://localhost:5000/issue-coupon/filter', camelToSnake(filters),
      {
        headers: {
          Authorization: token,
