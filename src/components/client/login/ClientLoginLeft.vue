<template>
    <div class="login-left">
      <h1>Login</h1>
      <!-- @submit.prevent 사용 -->
      <form class="login-form" @submit.prevent="handleLogin">
        <input type="text" placeholder="아이디" class="login-input" v-model="userId" />
        <input type="password" placeholder="비밀번호" class="login-input" v-model="password" />
        <button type="submit" class="login-button">로그인</button>
        <div class="social-login">
          <button type="button" class="kakao-login" @click="handleKakaoLogin">
            <img src="@/assets/icons/kakao.svg" alt="카카오" />
            카카오 로그인
          </button>
          <button type="button" class="google-login" @click="handleGoogleLogin">
            <img src="@/assets/icons/google.svg" alt="구글" />
            구글 로그인
          </button>
        </div>
        <div class="login-links">
          <a href="#" class="link">아이디 찾기</a>
          <span>|</span>
          <a href="#" class="link">비밀번호 찾기</a>
          <span>|</span>
          <a href="#" class="link" @click="$emit('show-login-signup')">회원가입</a>
        </div>
      </form>
    </div>
  </template>
  
  <script setup>
  import { ref } from 'vue';
  
  const userId = ref(''); // 사용자 아이디 입력값
  const password = ref(''); // 사용자 비밀번호 입력값
  
  // 로그인 버튼 처리
  const handleLogin = () => {
    console.log(`아이디: ${userId.value}, 비밀번호: ${password.value}`);
    // 부모 컴포넌트에 이벤트 전달
    emit('login', { userId: userId.value, password: password.value });
  };
  
  // 카카오 로그인 처리
  const handleKakaoLogin = () => {
    console.log('카카오 로그인');
    emit('social-login', 'kakao');
  };
  
  // 구글 로그인 처리
  const handleGoogleLogin = () => {
    console.log('구글 로그인');
    emit('social-login', 'google');
  };
  </script>
  
  <style scoped>
  /* Left 컴포넌트 스타일 */
  .login-left {
    border-top: 8px solid #7671f4;
    width: 50%;
    padding: 60px;
    background-color: #ffffff;
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
  
    h1 {
      font-weight: bold;
      margin-bottom: 50px;
      font-size: 30px;
      color: #7671f4;
    }
  
    .login-form {
      display: flex;
      flex-direction: column;
      gap: 15px;
  
      .login-input {
        width: 350px;
        padding: 10px;
        font-size: 14px;
        background-color: rgba(232, 240, 254, 0.4);
        border: 1.3px solid #e1dfdf;
        border-radius: 11px;
        box-shadow: 0px 3px 1px rgba(0, 0, 0, 0.2);
  
        &:focus {
          outline: none;
        }
      }
  
      .login-input::placeholder {
        color: rgba(114, 114, 114, 0.5);
        font-size: 15px;
      }
  
      .login-button {
        width: 100%;
        margin-top: 30px;
        padding: 10px;
        background-color: #7671f4;
        color: #fff;
        font-size: 17px;
        border: none;
        border-radius: 60px;
        cursor: pointer;
        box-shadow: 0px 3px 1px rgba(0, 0, 0, 0.2);
  
        &:hover {
          background-color: #5c57d6;
        }
      }
  
      .social-login {
        display: flex;
        flex-direction: column;
        gap: 15px;
  
        .kakao-login,
        .google-login {
          display: flex;
          align-items: center;
          gap: 90px;
          width: 100%;
          padding: 10px;
          font-size: 16px;
          font-weight: bold;
          border-radius: 60px;
          border: none;
          cursor: pointer;
          box-shadow: 0px 3px 1px rgba(0, 0, 0, 0.2);
          justify-content: flex-start; /* 왼쪽 정렬 */
        }
  
        .kakao-login {
          background-color: #fee500;
          color: #000;
          &:hover {
          background-color: #f0d801;
        }
        }
  
        .google-login {
          background-color: white;
          color: #000;
          &:hover {
            background-color: #f1f1f1;
        }
    }
  
        img {
          width: 20px;
          margin-left: 10px;
          height: 20px;
          object-fit: contain;
        }
      }
  
      .login-links {
        display: flex;
        justify-content: center;
        gap: 15px;
        margin-top: 20px;
  
        .link {
          color: #a8a8a8;
          text-decoration: none;
          font-size: 14px;
  
          &:hover {
            text-decoration: underline;
          }
        }
  
        span {
          color: #ccc;
        }
      }
    }
  }
  </style>
  