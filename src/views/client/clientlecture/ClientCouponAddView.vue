      ...
      <div class="clientlecturecoupon-layout">
        ...
        <main class="clientlecturecoupon-content">
          ...
          <table class="clientlecturecoupon-coupon-table">
            ...
            <tbody>
              <tr v-for="coupon in paginatedCoupons" :key="coupon.couponCode">
                <td>{{ coupon.lectureTitle }}</td>
                <td>{{ coupon.couponDiscountRate }}%</td>
                <td>₩{{ Math.floor(coupon.lecturePrice - (coupon.lecturePrice * (coupon.couponDiscountRate / 100))).toLocaleString() }}</td>
                <td>{{ coupon.couponCode }}</td>
                <td>{{ formatDate(coupon.couponStartDate) }}</td>
                <td>{{ formatDate(coupon.couponExpireDate) }}</td>
                <td>{{ coupon.activeState ? 'O' : 'X' }}</td>
              </tr>
            </tbody>
          </table>
...
try {
  ...
  const response = await axios.get(`http://localhost:5000/coupon/client/${tutorCode}/allcoupon`, {
