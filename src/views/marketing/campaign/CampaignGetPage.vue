  ...
  const truncatedContents = computed(() => {
    const maxLength = 200; // 원하는 최대 글자 수 설정
    if (!campaign.value.campaign_contents) return '';
    
    if (campaign.value.campaign_contents.length > maxLength) {
      return campaign.value.campaign_contents.slice(0, maxLength) + '...';
    }
    return campaign.value.campaign_contents;
  });
